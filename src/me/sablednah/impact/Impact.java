...
//	public static Map<Player, Boolean> pluginEnabled = null;
					...
					public void run() {
						try {
							...
							if (!VersionNew.contains(VersionOld)) {
								logger.warning("["+Impact.myName+"] " + VersionNew + " is available. You're using " + VersionOld);
								logger.warning("["+Impact.myName+"] http://dev.bukkit.org/server-mods/"+Impact.myName+"/");
							}
	...
	public void loadConfiguration() {
		...
		String headertext;
		headertext = "Default "+Impact.myName+" Config file\r\n\r\n";
	...
	public String getNewVersion(String VersionCurrent) throws Exception {
		String urlStr = "http://sablekisska.co.uk/asp/"+Impact.myName+"version.asp";
		try {

			URL url = new URL(urlStr);
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			HttpURLConnection.setFollowRedirects(true);
			conn.setRequestProperty("User-agent", "["+Impact.myName+" Plugin] " + VersionCurrent);
			String inStr = null;
